...
class GlobalExceptionHandlerTests {
	...
	@Test
	void apiExceptionReturnsItsStatusAndCode() throws Exception {
		mockMvc.perform(get("/__test/errors/api-exception"))
				.andExpect(status().isNotFound())
				.andExpect(jsonPath("$.code", is("NOT_FOUND")))
				.andExpect(jsonPath("$.message", is("thing missing")))
				.andExpect(jsonPath("$.path", is("/__test/errors/api-exception")));
	}

	@Test
	void validationErrorReturns400WithFieldDetails() throws Exception {
		mockMvc.perform(post("/__test/errors/validate")
